...
function buy(ev, index, one, two, three, last, flew, stock) {
  ...
  const handler = PaystackPop.setup({
    ...
    callback: (transaction) => {
      ...
      var starCountRef = database.ref(`money/${UID}`);
      starCountRef.once("value").then((snapshot) => {
        const data = snapshot.val();
        if (!data) {
      ...
      var starCountRef = database.ref(`totalMoney`);
      starCountRef.once("value").then((snapshot) => {
        const data = snapshot.val();
        if (!data) {
      ...
      var starCountRef = database.ref(`goods/${last}`);
      starCountRef.once("value").then((snapshot) => {
        const data = snapshot.val();
      ...
      var starCountRef = firebase.database().ref(`purchased/${UID}`);
      starCountRef.once("value").then((snapshot) => {
...
function pay(ev, name, price, params, flew, stock, data) {
  ...
  setTimeout(() => {
    ...
    const handler = PaystackPop.setup({
      ...
      amount: price * 100,
      callback: (transaction) => {
        var starCountRef = firebase.database().ref(`carts/${UID}`);
        starCountRef.once("value").then((snapshot) => {
          const data = snapshot.val() || [];

          const findCart = data.findIndex((member) => {
            if (member) {
              return (
                member.second == name &&
                member.first == flew &&
          ...
          if (findCart !== -1) {
            firebase
              ...
              .update({
                bought: +data[findCart].bought - 1,
                stock: +data[findCart].stock - 1,
              });
          }
        });
        ...
        var starCountRef = firebase.database().ref(`goods/${params}`);
        starCountRef.once("value").then((snapshot) => {
          const data = snapshot.val();
  ...
  const handler = PaystackPop.setup({
    ...
    callback: (transaction) => {
      ...
      starCountRef.once("value").then((snapshot) => {
        ...
        const number = data[UID] ? +data[UID].on + quantity : 1;
      ...
      setTimeout(() => {
        ...
        starCountRef.once("value").then((snapshot) => {
          ...
          const number = data[UID] ? +data[UID].on - quantity : 0;
      ...
      setTimeout(() => {
        ...
        var starCountRef = database.ref(`goods/${goodIndex}`);
        starCountRef.once("value").then((snapshot) => {
          const data = snapshot.val();
...
function viewReceipts() {
  ...
  var starCountRef = firebase.database().ref(`discount`);
  starCountRef.on("value", (snapshot) => {
    const slow = snapshot.val() ? snapshot.val() : false;
    const data = slow[UID] ? slow[UID] : [];

    var starCountRef = firebase.database().ref(`order`);
    starCountRef.on("value", (snapshot) => {
      const tall = snapshot.val() ? snapshot.val() : false;
      const fly = tall[UID] ? tall[UID] : [];

      var starCountRef = firebase.database().ref(`purchased`);
      starCountRef.on("value", (snapshot) => {
        const fish = snapshot.val() ? snapshot.val() : false;
    ...
    const handler = PaystackPop.setup({
      ...
      callback: (transaction) => {
        ...
        starCountRef.once("value").then((snapshot) => {
          ...
          const number = data[UID] ? +data[UID].on + 1 : 1;
        ...
        setTimeout(() => {
          ...
          starCountRef.once("value").then((snapshot) => {
            ...
            const number = data[UID] ? +data[UID].on - 1 : 0;
        ...
        setTimeout(() => {
          ...
          var starCountRef = database.ref(`goods/${index}`);
          starCountRef.once("value").then((snapshot) => {
            const data = snapshot.val();
            let newSold = +data.sold + +quantity;
    ...
    starCountRef.once("value").then((snapshot) => {
      ...
      const handler = PaystackPop.setup({
        ...
        callback: (transaction) => {
          ...
          var orderRef = firebase.database().ref(`totalDiscount`);
      ...
      const handler = PaystackPop.setup({
        ...
        callback: (transaction) => {
          ...
          setTimeout(() => {
            ...
            for (let index = indexDisc; index < indexDisc + 3; index++) {
              let starCountRef = database.ref(`goods/${index}`);
              starCountRef.once("value").then((snapshot) => {
                const data = snapshot.val();
