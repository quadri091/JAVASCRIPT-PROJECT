...
const products = [
    ...
    },
    {
        ...
        description: "Ultra-high speed SSD for lightning fast loading and deeper immersion.",
        image: "https://i.pinimg.com/736x/af/84/0c/af840cca03f57b593c5a312c8e377e87.jpg",
...
function addChange(index) {
    ...
    setTimeout(() => {
        const priPrice = proPri.innerHTML
            .replace("Price: ₦", "")
            .replaceAll(",", "")
            .replaceAll("₦", "")
            .replaceAll(" ", "")
        const priRat = proRat.innerHTML.replace("Ratings: ", "").replaceAll(" ", "")
        const priSold = proPur.innerHTML.replace("Purchased: ", "").replaceAll(" ", "")
        const priStock = proStock.innerHTML.replace("Stock: ", "").replaceAll(" ", "")



        const updatedData = {
            name: proName.innerHTML.replace("Name: ", "").trim(),
            image: proImg.src,
            stock: +priStock,
...
function addProduct(params) {
    ...
    setTimeout(() => {
        ...
        params.innerHTML = "Push Product"
        var starCountRef = firebase.database().ref(`goods`);
        starCountRef.once("value").then((snapshot) => {
            const data = snapshot.val();
            const priPrice = pushPrice.innerHTML
                .replace("Price: ₦", "")
                .replaceAll(",", "")
                .replaceAll("₦", "")
                .replaceAll(" ", "")
            const priRat = pushRat.innerHTML.replace("Ratings: ", "").replaceAll(" ", "")
            const priSold = pushSold.innerHTML.replace("Sold: ", "").replaceAll(" ", "")
            const priStock = pushStock.innerHTML.replace("Stock: ", "").replaceAll(" ", "")
    ...
    starCountRef.on("value", (snapshot) => {
        ...
        array.forEach((member) => {
            ...
            dispDiscount.innerHTML += `<div class="pur-element">
              ...
              <div class="pur-head" style="font-size: 14px;">
                <h4>${member.time}</h4>
                <h4>${member.user}</h4>
                <h4>${member.type}</h4>
                <h4>${member.date}</h4>
